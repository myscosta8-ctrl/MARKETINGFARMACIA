 ...
 * pede um `state` assinado ao servidor (que deriva a farmácia do próprio
 * JWT autenticado) antes de abrir o popup de OAuth da Meta — farmaciaId
 * aqui é só para exibição/contexto de UI, nunca é enviado para decidir
 * qual farmácia está sendo conectada (isso sempre vem do backend).
 */
export function PainelIntegracoesMeta({ farmaciaId: _farmaciaId, podeEditar }) {
  ...
  async function conectar(provedor) {
    ...
    // O state é gerado no servidor (Edge Function), nunca aqui — o
    // farmacia_id é derivado do JWT do admin autenticado do lado de lá,
    // impedindo que alguém adultere qual farmácia está sendo conectada.
    const { data: sessao } = await supabase.auth.getSession();
    const { data: resultado, error } = await supabase.functions.invoke('meta-actions', {
      body: { tipo: 'iniciar_conexao_oauth', provedor },
      headers: { Authorization: `Bearer ${sessao?.session?.access_token}` },
    });
    if (error || !resultado?.state) {
      logger.error('Falha ao iniciar conexão OAuth', error);
    ...
    const url = `https://www.facebook.com/v21.0/dialog/oauth?client_id=${appId}&redirect_uri=${encodeURIComponent(redirectUri)}&state=${encodeURIComponent(resultado.state)}&scope=${encodeURIComponent(escopo)}`;
