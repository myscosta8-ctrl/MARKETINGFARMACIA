...
import { publicarConteudoFacebook } from './service';
import { labelStatus, corStatus, STATUS_INTEGRACAO_LABEL } from './constants';

export default function FacebookPage() {
  const { perfil } = useAuth();
  const { pode_ver, pode_editar, carregando: carregandoPermissoes } = usePermissoes('facebook');
  ...
  async function carregarIntegracao() {
    ...
    const { data, error } = await supabase.from('integracoes').select('*').eq('provedor', 'facebook').maybeSingle();
    if (error) logger.error('Falha ao carregar status de integração do Facebook', error);
    setIntegracao(data);
  ...
  async function carregarConteudosElegiveis() {
    ...
    const { data: jaPublicados } = await supabase.from('facebook_publicacoes').select('conteudo_id');
  ...
  async function publicar(conteudoId) {
    ...
    try {
      await publicarConteudoFacebook({ conteudoId });
  ...
  if (!pode_ver) {
    return (
      <div className="max-w-2xl">
        <h1 className="font-display text-2xl text-ink-100">Facebook</h1>
        <p className="text-ink-500 text-sm mt-2">Você não tem permissão para acessar o módulo de Facebook.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl">
      <h1 className="font-display text-2xl text-ink-100">Facebook</h1>
      <p className="text-ink-500 text-sm mt-1">Publicação de conteúdos marcados com o canal Facebook — sem credencial oficial configurada ainda.</p>
      ...
      <p className="text-xs text-ink-500 mb-3">
        Só aparecem aqui conteúdos já marcados com o canal Facebook em{' '}
